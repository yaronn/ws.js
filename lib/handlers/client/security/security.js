...
SecurityClientHandler.prototype.send = function(ctx, callback) {
	...
	var security = utils.addElement(doc, header, this.security_ns, "o:Security", null);
...
SecurityClientHandler.prototype.AddTimestamp = function(doc, security) {		
	...
	var expires_time = new Date(created_time); 

	var expires_timespan = 5;
	...
	var expires = utils.addElement(doc, timestamp, this.utility_ns, "u:Expires", dateFormat(expires_time, "isoUtcDateTime"));
}
...
UsernameToken.prototype.applyMe = function(doc, security) {	
	...
	var token = utils.addElement(doc, security, security.security_ns, "o:UsernameToken", null);
	var username = utils.addElement(doc, token, security.security_ns, "o:Username", this.options.username);
	var password = utils.addElement(doc, token, security.security_ns, "o:Password", this.options.password);
