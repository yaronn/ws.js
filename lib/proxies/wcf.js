...
var sec = require('../handlers/client/security/security.js');
...
function Proxy(binding, url) {
	...
	this.send = function(message, action, callback) {		
		...
		ws.send(this.handlers, this.context, function(ctx) {callback(ctx.response, ctx)});
	}
}
...
function BasicHttpBinding(options) {
	...
	this.getCustomBinding = function() {
		...
		if (this.SecurityMode=="TransportWithMessageCredential"
			...
			channels.push(new SecurityBindingElement({AuthenticationMode: "UserNameOverTransport"}));						
		}
...
function WSHttpBinding(options) {
	...
	this.getCustomBinding = function() {
		...
		if (this.MessageEncoding && this.MessageEncoding.toLowerCase()=="mtom")
			channels.push(new MtomMessageEncodingBindingElement({MessageVersion: "Soap12WSAddressing10" }));
		else
			channels.push(new TextMessageEncodingBindingElement({MessageVersion: "Soap12WSAddressing10" }));		
