...
module.exports = {
	...
	"basic http binding correctly translates to custom binding when there are settings": function (test) {        				
			...
			test.equal("UsernameOverTransport", c.channels[0].AuthenticationMode, "wrong authentication mode");
			test.equal("Soap11", c.channels[1].MessageVersion, "wrong soap message version");
			test.equal("HttpsTransportBindingElement", utils.getTypeName(c.channels[2]), "wrong soap message version");
	...
	"wsHttp binding correctly translates to custom binding when there are settings": function (test) {        				
			...
			test.equal(3, c.channels.length, "wrong number of channels");
			test.equal("usernameOverTransport", c.channels[0].AuthenticationMode, "wrong authentication mode");
			test.equal("Soap12WSAddressing10", c.channels[1].MessageVersion, "wrong soap message version");
			test.equal("HttpsTransportBindingElement", utils.getTypeName(c.channels[2]), "wrong soap message version");
	...
	"custom binding correctly translated to handlers when there is addressing and security": function(test) {
		...
		test.equal("SecurityClientHandler", utils.getTypeName(handlers[0], "http handler not found"));	

		test.equal("yaron", handlers[0].tokens[0].options.username, "wrong username found");	

		test.equal("MtomClientHandler", utils.getTypeName(handlers[1], "http handler not found"));	
		test.equal("WsAddressingClientHandler", utils.getTypeName(handlers[2], "http handler not found"));	

		test.equal("http://schemas.xmlsoap.org/ws/2004/08/addressing", handlers[2].version, "wrong addressing version found");

		test.equal("HttpClientHandler", utils.getTypeName(handlers[3], "http handler not found"));
